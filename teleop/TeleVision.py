...
class OpenTeleVision:
    ...
    async def main_webrtc(self, session, fps=60):
        ...
        session.upsert @ WebRTCStereoVideoPlane(
                src="https://192.168.8.102:8080/offer",    #original code
                #src="https://192.168.123.250:8080/offer", # usr ljt add
                key="zed",
                aspect=1.33334,
                height = 8,
                position=[0, -2, -0.2],
    ...
    async def main_image(self, session, fps=60):
        session.upsert @ Hands(fps=fps, stream=True, key="hands", showLeft=False, showRight=False)
        #------usr ljt add------#
        print(f"self.img_array.shape on TeleVision:{self.img_array.shape}")
        print(f"img_width on TeleVision:{self.img_width}")
        half_width = int(self.img_width / 2.)
        overlap_ratio=0.6
        overlap_width = int(self.img_width / 2 * overlap_ratio)
        #------usr ljt add------#
        ...
        while True:
            ...
            [ImageBackground(
                # Can scale the images down.
                #display_image[::2, :self.img_width],   # original code
                display_image[:, :half_width + overlap_width],   # usr ljt add
                ...
                height = 8,
                #position=[0, -1, 3],   # original code
                position=[-2, 0, -5],   # usr ljt add
                # rotation=[0, 0, 0],
                layers=1, 
